...
function arbiterScore(b) {
  let s = 0;
  try {
    const a = BigInt(b.amount || '0'), E = BigInt('1000000000000000000');
    if (a > E * BigInt(10)) s += 40; else if (a > E) s += 20; else s += 5;
  } catch(e) { s += 5; }
...
export async function POST(req) {
  try {
    ...
    if (verdict.action === 'block') {
      return Response.json({ error: 'ARBITER_BLOCKED', score, verdict }, { status: 403 });
    }
    const res = await fetch(UNI, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) return Response.json({ error: 'API_ERROR', status: res.status, details: data, arbiter: { score, verdict } }, { status: res.status });
    const RN = {0:'CLASSIC',1:'DUTCH_LIMIT',2:'DUTCH_V2',3:'LIMIT_ORDER',4:'WRAP',5:'UNWRAP',6:'BRIDGE',7:'PRIORITY',8:'DUTCH_V3'};
    return Response.json({ ...data, routingName: RN[data.routing] || 'UNKNOWN', arbiter: { score, verdict } });
  } catch (e) {
